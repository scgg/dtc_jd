...
static int HelperProcRun (struct THelperProcParameter *args)
{
	...
	alarm(procTimeout);
	if (helperProc->Init(args->gid, dbConfig, gTableDef[0], args->role) != 0)
	{
...
int check_db_table(int gid, int role)
{
	...
	alarm(procTimeout);
	if (helper->Init(gid, dbConfig, gTableDef[0], role) != 0)
	{
		log_error ("%s", "helper process init failed");
		delete helper;
		alarm(0);
...
int main(int argc, char **argv)
{
	...
	while(!stop) {
		...
		WatchDogFork(helperProc->Name(), (int(*)(void*))HelperProcRun, (void *)&helperArgs);

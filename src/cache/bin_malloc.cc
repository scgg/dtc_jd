...
/*头信息包括：参见newmanwang的TTC内存分配器实现的分享*/
...
/*查看cache是否一致：在启动ttc，加载cache的时候，只要是需要写cache，就会设置不一致，防止ttc在运行时crash，重启后不经检查使用乱掉的内存*/
...
/*对于已经存在的IPC shared memory，ttc在启动后会将这个块内存作为cache，在这里检查这块cache的头信息，是否正确*/
...
/*realloc的主体逻辑*/
/*具体逻辑见newmanwang的TTC内存分配器的实现的分享*/

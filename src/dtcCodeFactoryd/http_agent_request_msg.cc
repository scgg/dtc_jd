...
CHttpAgentRequestMsg::CHttpAgentRequestMsg():m_isKeepAlive(false),m_isXForwardedFor(false),m_nHttpVersion(0)
{
}

int CHttpAgentRequestMsg::Decode(char *pBuff, int nLen)
{
	int nBodyLen=0;
	char szHead[64]={0};
...
string CHttpAgentRequestMsg::GetCommand() const
{
...
string CHttpAgentRequestMsg::GetBody() const
{
	log_debug("CHttpAgentRequestMsg::%s,m_strBody[%s]\n",__FUNCTION__,m_strBody.c_str());
	return m_strBody;
}

int CHttpAgentRequestMsg::HandleHeader(void *pPkg, int iBytesRecved, int *piPkgLen)
{
	...
	*piPkgLen = 0;
	...
	while (pEnd != NULL)
	{
		...
		{
			log_error("invalid http request. content header is %s", header);
			return -1;
		}

...
int CBlackList::init_blacklist(const unsigned max, const unsigned keytype, const unsigned expired)
{
...
int CBlackList::add_blacklist(const char *ptr, const unsigned vsize)
{
	if(!ptr)
		return 0;
...
int CBlackList::in_blacklist(const char *ptr)
{
	if(!ptr)
		return 0;
...
int CBlackList::try_expired_blacklist(void)
{
...
/* TODO: 统计top10 */
void CBlackList::stat_everything(const struct blslot *slot, const int add)
{
	/* add */
	if(add)
...
void CBlackList::dump_all_blslot(void)
{
	CKeyCodec key_codec(key_type);

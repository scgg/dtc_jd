...
CPluginAgentListenerPool::CPluginAgentListenerPool(void)
{
...
int CPluginAgentListenerPool::InitDecoder(int n, int idle)
{
    if(_thread[n]==NULL)
    {
        char name[16];
...
int CPluginAgentListenerPool::Bind()
{
    int succ_count = 0;

    int idle = CPluginGlobal::_idle_timeout;
    if(idle < 0)
    {
    ...
    int single = CPluginGlobal::_single_incoming_thread;
    int backlog = CPluginGlobal::_max_listen_count;
    int win = CPluginGlobal::_max_request_window;
...
//        }
        ...
        const char *addrStr = CPluginGlobal::_bind_address;
        if(addrStr==NULL)
        {
            continue;
        }
        ...
        errmsg = _sockaddr[i].SetAddress(addrStr, port);
        if(errmsg)
        {
        ...
        {	// DGRAM listener
            rbufsz =  CPluginGlobal::_udp_recv_buffer_size;
            wbufsz =  CPluginGlobal::_udp_send_buffer_size;
        }
        else
        {
...
int CPluginAgentListenerPool::Match(const char *name, int port)
{
    for(int i=0; i<MAXLISTENERS; i++)
    {
        if(_listener[i]==NULL)
...
int CPluginAgentListenerPool::Match(const char *name, const char *port)
{
    for(int i=0; i<MAXLISTENERS; i++)
    {
        if(_listener[i]==NULL)

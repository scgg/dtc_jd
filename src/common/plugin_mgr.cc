...
CPluginManager* CPluginManager::Instance (void)
{
    return CSingleton<CPluginManager>::Instance();
}

void CPluginManager::Destroy (void)
{
    return CSingleton<CPluginManager>::Destroy ();
}

int CPluginManager::open (int mode)
{
    ...
    _plugin_name = gConfig->GetStrVal("cache", "PluginName");
    ...
    CPluginGlobal::_plugin_conf_file = gConfig->GetStrVal("cache", "PluginConfigFile");
    ...
    //crate CPluginListenerPool object
    NEW (CPluginListenerPool, _plugin_listener_pool);
    if (NULL == _plugin_listener_pool)
    {
        log_error ("create CPluginListenerPool object failed, msg:%s", strerror(errno));
        return -1;
    }
    ...
    //create worker pool
    if (create_worker_pool (gConfig->GetIntVal("cache", "PluginWorkerNumber")) != 0)
    {
    ...
    {
        if (create_plugin_timer(gConfig->GetIntVal("cache", "PluginTimerNotifyInterval", 10)) != 0)
        {
...
int CPluginManager::load_plugin_depend (int mode)
{
    char* error     = NULL;
...
int CPluginManager::register_plugin (const char* file_name)
{
    char*   error       = NULL;
    int     ret_code    = -1;
...
void CPluginManager::unregister_plugin (void)
{
    if (NULL != _dll)
    {
        if (NULL != _dll->handle)
...
int CPluginManager::create_worker_pool (int worker_number)
{
...
int CPluginManager::create_plugin_timer (const int interval)
{
    char name[32]               = {'\0'};

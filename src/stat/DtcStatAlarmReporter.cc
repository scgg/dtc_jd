...
void DtcStatAlarmReporter::PostAlarm(const std::string& strAlarmContent)
{
	...
	oss << m_strReportURL  << "?moduleid=" << m_ddwModuleId <<  "&ip=" << m_DtcIp << "&cellphonenum=" << m_CellPhoneList << "&content=" << strAlarmContent;		
	CurlHttp curlHttp;
	BuffV buf;
	...
	if (0 != ret)
	{
		log_error("curl http request fail ,ret code is %d", ret);		
		return;
	}

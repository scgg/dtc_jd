...
#include "http_plugin_error_code.h"
#include <string.h>
#include <strings.h>
#include <stdlib.h>
#include <stdio.h>
...
CHttpPluginResponseMsg::CHttpPluginResponseMsg():m_nHttpVersion(0),m_isKeepAlive(false),m_nStatusCode(0)
{
}

void CHttpPluginResponseMsg::SetCode(int nCode)
{
	m_nStatusCode = nCode;
}

void CHttpPluginResponseMsg::SetKeepAlive(bool isKeepAlive)
{
	m_isKeepAlive = isKeepAlive;
}

void CHttpPluginResponseMsg::SetBody(const string &strResponse)
{
	m_strResponse = strResponse;
}

void CHttpPluginResponseMsg::SetHttpVersion(int nHttpVersion)
{
	m_nHttpVersion = nHttpVersion;
}


const string & CHttpPluginResponseMsg::Encode()
{
	return Encode(m_nHttpVersion,m_isKeepAlive,m_strResponse);
}

const string & CHttpPluginResponseMsg::Encode(int nHttpVersion,bool isKeepAlive,int nCode)
{
    char bufCode[10];
...
const string & CHttpPluginResponseMsg::Encode(int nHttpVersion,bool isKeepAlive,const string &strResponse)
{

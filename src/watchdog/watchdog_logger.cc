...
int StartFaultLogger(CWatchDog *wdog) {
	...
	display = gConfig->GetStrVal("cache", "FaultLoggerMode");
	...
	if(pid==0) {
		...
		CThread *loggerThread = new CThread("faultlogger", CThread::ThreadTypeProcess);
		loggerThread->InitializeThread();
	...
	CFaultHandler::Initialize(mode>=2);
	Set_LD_PRELOAD();

	CWatchDogObject *obj = new CWatchDogObject(wdog, "FaultLogger", pid);
	obj->AttachWatchDog();
	return 0;
}

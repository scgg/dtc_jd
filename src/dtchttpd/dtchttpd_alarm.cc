...
bool Alarm::ReportToPlatform(int alarm_type, uint32_t val1, uint32_t val2)
{
	...
	switch(alarm_type)
	{
		case WAIT_PID_FAIL:
			{
				innerBody["alarm_title"] = "dtchttpd wait pid";
				oss << "dtchttpd father process wait pid fail";
				break;
			}
		case KILL_BY_SIGNAL:
			{
				innerBody["alarm_title"] = "dtchttpd kill by signal";
				oss << "dtchttpd child process killed by signal " << val1;
				break;
			}
		case FORK_ERROR:
			{
				innerBody["alarm_title"] = "dtchttpd fork error";
				oss << "dtchttpd father process fork child process error";
				break;
			}
		case QUEUE_CONGEST:
			{
				...
				innerBody["alarm_title"] = "dtchttpd queue congest";
				oss << "dtchttpd child process queue size is " << val1 << " has use " << val2;
				break;
			}
		case CPU_RATE_HIGH:
			{
				innerBody["alarm_title"] = "dtchttpd cpu rate high";
				oss << "dtchttpd ";

...
DTCt_dtc_example::DTCt_dtc_example(){
	init();
}
...
void DTCt_dtc_example::clearErr(){
	errCode = 0;
	errMsg  = "";
	errFrom = "";
	dtcCode = 0;
 ...
 * @return bool
 */
bool DTCt_dtc_example::purge(string key){
	int ret = 0;
	clearErr();
 ...
 * @param key    dtc key 字段值(fieldvalue)
 */
bool DTCt_dtc_example::delet(string key){
	int ret = 0;
	clearErr();
 ...
 */
bool DTCt_dtc_example::get(string key, vector<Field>& need, vector<Field>& resultSet){
	int ret = 0;
	clearErr();
 ...
 */
bool DTCt_dtc_example::update(string key, vector<Field>& fieldList){
	int ret = 0;
	clearErr();
 ...
 */
bool DTCt_dtc_example::insert(string key, vector<Field>& fieldList){
	int ret = 0;
	clearErr();
 ...
 */
int DTCt_dtc_example::getAffectRows(){
	return affectRows;
}
 ...
 * @return int AffectedRows
 */
bool DTCt_dtc_example::checkKey(string key){
	clearErr();
	switch(keyType){
	case Signed:
	case Unsigned:
 ...
 */
bool DTCt_dtc_example::check(vector<Field>& fieldList){
	clearErr();
	for(vector<Field>::iterator iter = fieldList.begin(); iter!= fieldList.end(); ++iter){
	int i = 0;
	Field fd = *iter;
 ...
 * 获取该字段在DTCConfig.fieldcfg 数组中的 index
 */
int DTCt_dtc_example::GetFlag(string fieldName){
	int flag = 0;
	int i=0;
	int itmp = sizeof(fieldcfg)/(sizeof(fieldcfg[0][0])*4);
	for(; i<itmp; i++){

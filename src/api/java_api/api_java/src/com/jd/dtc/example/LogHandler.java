...
public class LogHandler extends Formatter{
	...
	public static Logger initLog(String name){
		if( instance == null )
		{
			...
			if(TestWorker.level.equals("0") )
				logger.setLevel(Level.OFF);
			else if(TestWorker.level.equals("1") )
				logger.setLevel(Level.SEVERE);
			else if(TestWorker.level.equals("2") )
				logger.setLevel(Level.WARNING);
			else if(TestWorker.level.equals("3") )
				logger.setLevel(Level.INFO);
			else if(TestWorker.level.equals("4") )
				logger.setLevel(Level.CONFIG);
			else if(TestWorker.level.equals("5") )
				logger.setLevel(Level.FINE);
			else if(TestWorker.level.equals("6") )
				logger.setLevel(Level.FINER);
			else if(TestWorker.level.equals("7") )

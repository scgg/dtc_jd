...
int main(int argc,char* argv[])
{
	...
	stServer.SetTableName("t_dtc_example");//设置ttc的表名，与在dtc.jd.com网站上申请dtc时指定的表名即tablename应该一样
	stServer.SetAddress("192.168.214.62", "10109");//设置的ttc的ip和端口
    ...
    { 
		printf ("uin[%u] ttc execute get error: %d, error_from:%s, msg:%s\n",
			uid,//出错的key是多少
			iRet,//错误码为多少
	...
	for(int i=0;i<=stResult.NumRows();++i)
	{
		...
		if(iRet < 0)
		{
			printf ("uid[%lu] ttc fetch row error: %d\n", uid, iRet);
			fflush(stdout);
			return(-3);
		}
		//如果一切正确，则可以输出数据了

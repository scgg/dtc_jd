...
//WRAPPER for log
__EXPORT
void TencentTableCache::init_log (const char *app, const char *dir)
{
	::_init_log_(app, dir);
}

__EXPORT
void TencentTableCache::set_log_level(int n)
{
	::_set_log_level_(n);
}

__EXPORT
int TencentTableCache::set_key_value_max(unsigned int count)
{
...
#define _DECLACT_(f0,t0,f1,f2,t1) \
	...
	CAST(NCRequest, r); \
	return r->f1(n, DField::f2, DField::t1, CValue::Make(v)); \
    }
#define _DECLACT2_(f0,t0,f1,f2,t1) \
    __EXPORT \
    int Request::f0(const char *n, t0 v, int l) { \
	CAST(NCRequest, r); \
	return r->f1(n, DField::f2, DField::t1, CValue::Make(v,l)); \
    }
...
{
    /*bufix:3774451 r cast to CTask will result timestamp is zero
     * must cast to NCResult
     * fix by foryzhou@tencent.com*/
	//CAST(CTask, r);
	CAST(NCResult, r);

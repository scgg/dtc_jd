...
typedef struct fd_setT {
	fd_mask fds_bits[howmany(FD_SETSIZE, NFDBITS)];
} bt_set1;


#define _fdset_mask(n)  ((fd_mask)1 << ((n) % NFDBITS))
#define FD_SET1(n, p)    ((p)->fds_bits[(n)/NFDBITS] |= _fdset_mask(n))
#define FD_CLR1(n, p)    ((p)->fds_bits[(n)/NFDBITS] &= ~_fdset_mask(n))
#define FD_ISSET1(n, p)  ((p)->fds_bits[(n)/NFDBITS] & _fdset_mask(n))



static inline int FIELD_ISSET(int id, const uint8_t *mask) {
	return FD_ISSET1(id, (const bt_set1 *)mask);
}

static inline void FIELD_SET(int id, uint8_t *mask) {
	FD_SET1(id, (bt_set1 *)mask);
}
static inline void FIELD_CLR(int id, uint8_t *mask) {
	FD_CLR1(id, (bt_set1 *)mask);
}

...
static unsigned short name2port(const char *name, int type) {
	...
	if(p) {
		...
		localname[len] = 0;
		result=getservbyname(localname, proto);
	} else {
...
static unsigned int ip2addr(const char *name) {
	...
	if (p==NULL)
	{
		if( (addrSrv.sin_addr.S_un.S_addr=inet_addr(name))<= 0 )
			return INADDR_NONE;
	}
	else
	{
	...
	unsigned int v = ntohl(addrSrv.sin_addr.S_un.S_addr);
	if(v < 0x0100000)
		return INADDR_NONE;

	return addrSrv.sin_addr.S_un.S_addr;
...
void CSocketAddress::BuildNameIpv4(void) {
	...
	if(in4->sin_addr.s_addr==0) {
		...
		ip=inet_ntoa(in4->sin_addr);
		//inet_ntoa(AF_INET, &in4->sin_addr, ip, sizeof(ip));
	}
	sprintf(sockname, "%s:%d/%s", ip, ntohs(in4->sin_port), socktype==SOCK_STREAM?"tcp":"udp");
//	printf("BuildNameIpv4---\n");
}

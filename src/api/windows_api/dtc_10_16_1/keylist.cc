...
int NCKeyInfo::Equal(const NCKeyInfo &other) const {
	...
	for(int i=0; i<n; i++) {
		// key name mis-match
		if(stricmp(keyName[i], other.keyName[i]) != 0)
			return 0;
	}
	return 1;
}
...
int NCKeyValueList::AddValue(const char* name, const CValue &v, int type)
{
	const int kn = KeyFields();
	...
	if(row >= kcount){
		if((val=(CValue*)realloc(val, (kcount+1)*kn*sizeof(CValue))) == NULL)
			throw std::bad_alloc();
		memset(&Value(row, 0), 0, kn*sizeof(CValue));
		kcount++;
	}
	CValue &slot = Value(row, col);
	switch(KeyType(col)){
		case DField::Signed:
		case DField::Unsigned:
			slot = v;

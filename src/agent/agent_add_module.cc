...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        fprintf(stderr, "usage: %s admin_addr module listen_address access_token\n", argv[0]);
        return 1;
    }
    ...
    ttc::agent::AddModuleRequest request;
    request.set_module(module);
    request.set_addr(argv[3]);
	request.set_accesstoken(argv[4]);
    ...
    header->cmd = AC_AddModule;
    request.SerializeWithCachedSizesToArray(buf + sizeof(ProtocolHeader));

    if(send_n(s, (char *)buf, sizeof(ProtocolHeader) + byteSize) < 0)
    {
        perror("send error");
        return -1;
    }
    ...
    fprintf(stderr, "add module failed: %s\n", reply.msg().c_str());
    return 1;
}

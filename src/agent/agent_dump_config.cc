...
int main(int argc, char **argv)
{
    if(argc != 2)
    {
        fprintf(stderr, "usage: %s admin_addr\n", argv[0]);
        return 1;
    }
    ...
    ttc::agent::QueryAgentStatus request;
    request.set_id(1);
    ...
    {
        printf("dump config success\n");
        printf("%s\n", reply.msg().c_str());
        return 0;
    }

    fprintf(stderr, "dump config failed: %s\n", reply.msg().c_str());
    return 1;
}

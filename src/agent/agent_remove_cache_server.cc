...
int main(int argc, char **argv)
{
    if(argc != 4)
    {
        fprintf(stderr, "usage: %s admin_addr module name\n", argv[0]);
        return 1;
    }
    ...
    ttc::agent::RemoveCacheServerRequest request;
    ...
    header->cmd = AC_RemoveCacheServer;
    ...
    fprintf(stderr, "remove server failed: %s\n", reply.msg().c_str());
    return 1;
}

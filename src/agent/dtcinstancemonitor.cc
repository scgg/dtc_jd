...
int writebackcompensiteitems() {
	...
	struct stat sb;
	struct compensiteitem *ci_list;
	int ci_fd = open(COMPENSITELIST_FILE, O_RDWR | O_CREAT);
	if (ci_fd == -1) {
		log_error("load compensite list from file failed");
		return -1;
	}
...
int loadinststat() {
	...
	int inst_stat_fd = open(DTCINSTANCESTATUS_FILE, O_RDWR | O_CREAT);
	if (inst_stat_fd == -1) {
...
int loadconfig() {
	...
	snprintf(buf, sizeof(buf) - 1,
			"select business_id,instance_id,ip,port from dtc_instance;");
	int rtn = Connect();
	if (rtn != 0) {
		return -1;
	}
...
bool ReportToAlarm(std::string ip) {
	...
	Json::Value body;
	body["app_name"] = "dtc_instance_monitor";
	body["alarm_type"] = "sms";
	body["data"] = innerBody;
